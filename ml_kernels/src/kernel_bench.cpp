...
public:
    const char *name() const override { return "softmax_v3_unroll4"; }
    void run() override {
        ml_kernels::softmax_v3_unroll4(inputs_[current_idx_].data(), outputs_[current_idx_].data(), inputs_[0].size());
        current_idx_ = (current_idx_ + 1) % pool_size_;
    }
};
