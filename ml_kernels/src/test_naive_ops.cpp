...
void test_softmax_v6() {
    ...
    std::vector<float> input = {1.0f, 2.0f, 3.0f, 4.0f, 5.0f, 1.0f, 2.0f, 3.0f, 4.0f, 5.0f,
                                ...
                                1.0f, 2.0f, 3.0f, 4.0f, 5.0f, 1.0f, 2.0f, 3.0f, 4.0f, 5.0f,
                                1.0f, 2.0f, 3.0f, 4.0f, 5.0f, 1.0f, 2.0f, 3.0f, 4.0f, 5.0f,
                                1.0f, 2.0f}; // 72 elements to trigger remainder loops
    std::vector<float> output_naive(input.size());
    ...
    for (size_t i = 0; i < input.size(); ++i) {
        if (std::abs(output_naive[i] - output_v6[i]) > 1e-4) {
            std::cerr << "Mismatch at index " << i << ": expected " << output_naive[i] << " got " << output_v6[i] << std::endl;
            exit(1);
        }
    }
