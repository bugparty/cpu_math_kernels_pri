...
inline __m256 exp256_ps_v3(__m256 x) {
    ...
    // Single FMA for r = x - n*ln2, taking advantage of softmax shift-invariance
...
// ⚡ Thunderbolt: AVX2 Vectorized Softmax with single-FMA exp256 and mixed unrolling
// Target: AVX2 (Haswell+)
// Reason: Combines the ln2 components of the range reduction into a single FMA since softmax shift invariance
// tolerates the slight precision loss. The max and normalize phases are unrolled 8x to perfectly
// saturate execution ports (e.g. max_ps has 4 cycle latency and 0.5 cycle throughput),
// while the complex exp phase is kept at 4x unroll to prevent YMM register spilling.
// Expected gain: ~5-10% throughput improvement over softmax_v5.
inline void softmax_v6(const float *input, float *output, std::size_t n) {
    ...
    // 2. Compute exp and sum (4x unroll to avoid register spilling)
    i = 0;
    __m256 sum0 = _mm256_setzero_ps();
    __m256 sum1 = _mm256_setzero_ps();
    __m256 sum2 = _mm256_setzero_ps();
