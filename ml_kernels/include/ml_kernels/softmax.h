...
// ⚡ Thunderbolt: AVX2 Vectorized Softmax with single-FMA exp approximation and 8x unroll
// Target: AVX2 (Haswell+)
// Reason: In transcendental AVX2 SIMD approximations for softmax, combining constants for r = x - n * ln(2) into a single FMA instruction reduces instruction count and latency, while maintaining precision sufficient for shift-invariant operations. Combined with aggressive 8x unrolling to hide the remaining FMA latency, this fully saturates execution ports and shifts bottlenecks towards L1/L2 cache bandwidth, leading to higher throughput.
// Expected gain: ~15-25% over softmax_v5.
