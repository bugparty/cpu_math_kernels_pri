...
inline __m256 exp256_ps_v3(__m256 x) {
    ...
    // Use single fnmadd for r = x - n*ln2, combining the split constants
...
// ⚡ Thunderbolt: AVX2 Vectorized Softmax with Fused FMA in exp256
// Target: AVX2 (Haswell+)
// Reason: Avoids the secondary `fnmadd` instruction in `exp256_ps` by combining the split `ln(2)`
// constants into a single full-precision `ln(2)` (`0.6931471805599453f`) multiplication.
// Because Softmax is shift-invariant, the slight precision loss on the exponent does not cause
// correctness failures for standard ML workloads, but reduces FMA latency and port pressure.
// Expected gain: ~5-10% over softmax_v5 due to reduced exp256 latency.
