...
// ⚡ Thunderbolt: AVX2 Vectorized Softmax with FMA-optimized exp256 + 8x Unrolling
// Target: AVX2 (Haswell+)
// Reason: Using a single-FMA for the r = x - n*ln(2) approximation reduces register pressure, allowing aggressive 8x unrolling across all phases (max, exp+sum, normalize) to better hide instruction latencies and fully saturate execution ports.
// Expected gain: ~10-15% throughput over softmax_v5 due to higher unrolling.
