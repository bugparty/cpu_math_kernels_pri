...
inline __m256 exp256_ps_v3(__m256 x) {
    ...
    // Use fnmadd to do r = x - n*ln2, combining the two steps into one
...
// Target: AVX2 (Haswell+)
// Reason: By combining the two constants for `r = x - n * ln(2)` into a single FMA operation, we save an instruction and reduce latency on the critical path of the exponentiation step without sacrificing numerical output accuracy within ML tolerance levels (1e-4).
