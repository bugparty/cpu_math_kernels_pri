...
// Target: AVX2 (Haswell+)
// Reason: In AVX2 math kernels (like Softmax), aggressive 8x unrolling across all map-reduce phases effectively hides latencies without spilling YMM registers, perfectly matching the 4-cycle latency of max and add reductions while interleaving the multi-FMA approximations of exp256. This perfectly saturates the execution ports and transitions the kernel from latency-bound to throughput-bound.
// Expected gain: ~6% throughput over softmax_v5 on large memory-bound workloads (N=1048576, Fixed Memory).
