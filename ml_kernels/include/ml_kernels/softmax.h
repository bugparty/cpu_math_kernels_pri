...
inline __m256 exp256_ps_v3(__m256 x) {
    ...
    // Combine constants for r = x - n*ln2 into a single instruction
    // r = x - n * ln2
    // using fnmadd: -n * ln2 + x
    __m256 r = _mm256_fnmadd_ps(n, _mm256_set1_ps(0.6931471805599453f), x); // Full precision ln(2)
...
// Target: AVX2 (Haswell+)
// Reason: Uses a single combined constant for `ln(2)` in `fnmadd` avoiding a second FMA,
// and unrolls all loops 8x to fully saturate registers and execution ports, particularly for the expensive `exp256` and `max` operations.
// Expected gain: higher throughput due to better pipeline utilization.
