...
// ⚡ Thunderbolt: AVX2 Vectorized Softmax with Heterogeneous Unrolling (8x-4x-8x)
// Target: AVX2 (Haswell+)
// Reason: In multi-phase AVX2 kernels, simpler phases like max reduction and normalization can safely be unrolled 8x to fully saturate execution ports.
// However, the exponentiation phase uses Horner's scheme, which requires many YMM registers for constants. Unrolling the exp phase 8x would cause register spilling.
// Using an 8x-4x-8x heterogeneous unroll avoids spilling while maximizing throughput for the simpler phases.
// Expected gain: ~5-10% over softmax_v5 on large inputs.
