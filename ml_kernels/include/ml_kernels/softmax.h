...
// Target: AVX2 (Haswell+)
// Reason: Replaces split precision `ln(2)` range reduction with a single FMA. Since softmax normalizes by the sum,
// slight precision losses from using a single FMA cancel out during division, remaining within 1e-4 tolerance.
// Expected gain: ~5-10% throughput over softmax_v5 due to shorter dependency chain in exp.
