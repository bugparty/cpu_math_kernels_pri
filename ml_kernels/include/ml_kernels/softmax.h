...
inline __m256 exp256_ps_v3(__m256 x) {
    ...
    // Use fnmadd to do r = x - n*ln2, using a combined FMA instead of two for ln2
...
// Target: AVX2 (Haswell+)
// Reason: Replaces the two-instruction split for `ln(2)` in range reduction with a single FMA step.
// The single FMA step combines the constants, reducing port pressure and latency without exceeding
// numerical tolerance (1e-4) due to shift-invariance of softmax.
