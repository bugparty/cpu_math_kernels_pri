...
// ⚡ Thunderbolt: AVX2 Vectorized Softmax with 8x Unrolling and Single-FMA Range Reduction
// Target: AVX2 (Haswell+)
// Reason: `softmax_v5` is limited by register pressure and the 2-FMA latency of the exact ln(2) range reduction split.
// Using a single-FMA range reduction (sacrificing exact IEEE precision for shift-invariant Softmax) drops register pressure,
// allowing aggressive 8x unrolling to perfectly hide instruction latency across all phases (Max, Exp, Sum, Normalize).
