...
// ⚡ Thunderbolt: AVX2 Vectorized Softmax with single-FMA range reduction and 8x unrolled I/O
// Target: AVX2 (Haswell+)
// Reason: `_mm256_max_ps` and normalization multiplies are high-throughput and benefit from 8x unrolling to hide their 4-cycle latencies.
// The exponential phase remains 4x unrolled to avoid spilling YMM registers, but `exp256` reduces the two FMA sequence for `r = x - n*ln2`
// to a single FMA using a slightly less precise `ln2` representation, reducing port pressure.
// Expected gain: ~5% throughput over softmax_v5 due to fewer instructions and better latency hiding on the max/normalize passes.
