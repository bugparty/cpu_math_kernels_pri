...
inline __m256 exp256_ps_v3(__m256 x) {
    ...
    // Use a single fnmadd to do r = x - n*ln2, saving an instruction.
    // 0.6931471805599453f is the full ln(2) constant.
...
// ⚡ Thunderbolt: AVX2 Vectorized Softmax with single-FMA exp256 range reduction
// Target: AVX2 (Haswell+)
// Reason: Replaces the split ln(2) subtraction (r = x - n*ln2_hi - n*ln2_lo) with a single FMA operation
// using the full ln(2) constant. Since Softmax is shift-invariant, the slight loss in precision from not
// splitting ln(2) stays well within typical ML numerical tolerances (1e-4) while saving an instruction
// per loop iteration and improving overall throughput.
// Expected gain: ~5-10% over softmax_v5.
