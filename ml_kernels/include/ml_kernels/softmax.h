...
inline __m256 exp256_ps_v3(__m256 x) {
    ...
    // Combine constants for r = x - n * ln(2)
    __m256 r = _mm256_fnmadd_ps(n, _mm256_set1_ps(0.6931471805599453f), x); // ln(2) = 0.69314718...
...
// Target: AVX2 (Haswell+)
// Reason: Avoids `round_ps` by leveraging `cvtps_epi32` rounding mode, replaces Estrin's scheme with Horner's, and combines constants for range reduction (r = x - n * ln(2)) into a single FMA.
// When unrolled 4x, the independent Horner chains interleave perfectly, saturating execution ports and hiding latency better than Estrin, leading to higher throughput. The combined FMA reduces the latency by 1 FMA step.
// Expected gain: ~5-15% over softmax_v5.
