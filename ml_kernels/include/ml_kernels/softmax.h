...
// Target: AVX2 (Haswell+)
// Reason: `_mm256_max_ps` and `_mm256_add_ps` have 4-cycle latencies. A 4x unroll only issues 4 instructions, leaving execution ports idle.
// Unrolling 8x maintains 8 independent accumulators across all three phases (max reduction, exp/sum, normalization),
// perfectly matching the latency and fully saturating the execution ports without spilling YMM registers.
// Expected gain: ~10-15% throughput over 4x unroll (softmax_v5) on large arrays.
