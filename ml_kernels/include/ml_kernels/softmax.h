...
inline __m256 exp256_ps_v3(__m256 x) {
    ...
    // ⚡ Thunderbolt: combine constants for r = x - n * ln(2) into a single FMA
    // 0.693145751953125f + 1.428606765330187e-06f = 0.6931471805599453f
...
// ⚡ Thunderbolt: AVX2 Vectorized Softmax with single FMA reduction exp256
// Target: AVX2 (Haswell+)
// Reason: Uses exp256_ps_v3 which combines the `r = x - n * ln(2)` reduction constants into a single FMA instruction.
// This saves one FMA operation per exponential evaluated, translating to 4 fewer instructions per 32-element unrolled loop iteration,
// keeping execution ports less saturated without sacrificing meaningful precision within ML tolerances.
