...
inline __m256 exp256_ps_v3(__m256 x) {
    ...
    // Single FMA for range reduction instead of splitting ln(2)
...
// ⚡ Thunderbolt: AVX2 Vectorized Softmax with single-FMA exp256
// Target: AVX2 (Haswell+)
// Reason: Combines constants for `r = x - n * ln(2)` into a single FMA instruction instead of splitting `ln(2)`. This reduces instruction count and latency on the critical path, while keeping ML-acceptable numerical precision due to shift-invariant operations.
