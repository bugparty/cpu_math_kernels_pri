...
inline __m256 exp256_ps_v3(__m256 x) {
    ...
    // Combine ln2 split into single FMA
...
// Replaces the two-instruction range reduction (r = x - n*ln2_hi - n*ln2_lo) with a single `_mm256_fnmadd_ps` instruction.
// Since x is already shift-reduced relative to the maximum element, the small precision loss from using a single
// ln2 constant doesn't affect the final float precision of softmax up to 1e-4 tolerance.
// Expected gain: ~5-10% over softmax_v5 due to reduced instruction count and latency on the critical path.
