...
// ⚡ Thunderbolt: AVX2 Vectorized Softmax with 8x-4x-8x heterogeneous unrolling
// Target: AVX2 (Haswell+)
// Reason: Avoids register spilling during exp computation. Pass 1 (max reduction) and Pass 3 (normalization)
// benefit from 8x unrolling to perfectly interleave instructions. However, Pass 2 uses `exp256_ps_v2` with Horner's
// method, which consumes many constants and intermediate variables. Unrolling Pass 2 by 8x maxes out the 16 YMM
// registers and causes register spilling. Thus, we keep Pass 2 at a 4x unroll to maintain ideal ILP while
// pushing Passes 1 and 3 to 8x to saturate execution ports.
// Expected gain: ~10% over softmax_v5 on large arrays.
inline void softmax_v6(const float *input, float *output, std::size_t n) {
    ...
    // 2. Compute exp and sum (4x unroll avoids register spilling for Horner's exp256)
    i = 0;
    __m256 sum0 = _mm256_setzero_ps();
    __m256 sum1 = _mm256_setzero_ps();
    __m256 sum2 = _mm256_setzero_ps();
