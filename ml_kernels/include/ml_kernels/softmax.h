...
inline __attribute__((target("avx2,fma"))) __m256 exp256_ps_v3(__m256 x) {
    x = _mm256_max_ps(x, _mm256_set1_ps(-87.3f));
    __m256 x_log2e = _mm256_mul_ps(x, _mm256_set1_ps(1.4426950408889634f));
    ...
    // ⚡ Thunderbolt: combine r = x - n * ln(2) into a single FMA to reduce register pressure and instructions.
    // 0.6931471805599453 is ln(2).
...
// ⚡ Thunderbolt: AVX2 Vectorized Softmax with single-FMA exp256 and aggressive 8x unrolling
// Target: AVX2 (Haswell+)
// Reason: Reducing register pressure in exp256 allows for 8x unrolling (64 elements) without spilling,
// fully hiding instruction latencies and maximizing L1 bandwidth utilization.
// Expected gain: ~10-20% throughput over v5 on large inputs.
inline __attribute__((target("avx2,fma"))) void softmax_v6(const float *input, float *output, std::size_t n) {
    if (n == 0) return;
