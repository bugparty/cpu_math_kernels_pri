...
// ⚡ Thunderbolt: AVX2 Vectorized Softmax with single-constant range reduction for exp
// Target: AVX2 (Haswell+)
// Reason: Replaces the split-constant ln(2) subtraction in exp256 with a single FMA using `0.6931471805599453f`.
// This removes one high-latency FMA instruction from the critical path of the exponentiation routine,
// directly boosting throughput. The resulting minor precision loss is well within the 1e-4 tolerance
// required for typical ML softmax implementations due to shift-invariance.
// Expected gain: ~5-10% throughput over softmax_v5.
__attribute__((target("avx2,fma")))
inline void softmax_v6(const float *input, float *output, std::size_t n) {
    ...
    __m128 lo_max = _mm256_castps256_ps128(max0);
    __m128 hi_max = _mm256_extractf128_ps(max0, 1);
    lo_max = _mm_max_ps(lo_max, hi_max);
    __m128 shuf_max = _mm_shuffle_ps(lo_max, lo_max, _MM_SHUFFLE(2, 3, 0, 1));
    lo_max = _mm_max_ps(lo_max, shuf_max);
    shuf_max = _mm_shuffle_ps(lo_max, lo_max, _MM_SHUFFLE(1, 0, 3, 2));
    lo_max = _mm_max_ps(lo_max, shuf_max);
    float max_val = _mm_cvtss_f32(lo_max);
    ...
    __m128 lo_sum = _mm256_castps256_ps128(sum0);
    __m128 hi_sum = _mm256_extractf128_ps(sum0, 1);
    lo_sum = _mm_add_ps(lo_sum, hi_sum);
    __m128 shuf_sum = _mm_shuffle_ps(lo_sum, lo_sum, _MM_SHUFFLE(2, 3, 0, 1));
    lo_sum = _mm_add_ps(lo_sum, shuf_sum);
    shuf_sum = _mm_shuffle_ps(lo_sum, lo_sum, _MM_SHUFFLE(1, 0, 3, 2));
    lo_sum = _mm_add_ps(lo_sum, shuf_sum);
    float sum_val = _mm_cvtss_f32(lo_sum);
