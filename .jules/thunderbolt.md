...
## 2024-06-13 - Shift-invariant Single-FMA in Softmax and Mixed Loop Unrolling
**Learning:** For shift-invariant operations like softmax (where adding a constant to all inputs yields the same result), exact precision on intermediate range reduction values (`x - n*ln2`) is not required as long as it's proportional. Combining the split constants for `ln(2)` into a single FMA instruction saves one instruction per element and improves overall throughput while keeping errors safely within ML numerical tolerances (e.g., 1e-4). Additionally, complex routines like `exp256_ps` should not be unrolled as deeply as simpler loops (e.g., max reduction, normalization). Microbenchmarks showed 8x unroll for `max/normalize` saturated execution ports better, but 8x unroll on the `exp` phase caused YMM register spilling that degraded performance; 4x unroll for `exp` optimally interleaved the Horner scheme.
**Evidence:** `softmax_v6` achieves 4.48 GFLOP/s vs `softmax_v5`'s 4.19 GFLOP/s (~7% improvement) on N=1048576 (Fixed Memory) on GitHub Actions CI runner CPU while preserving correctness bounds.
**Action:** When working on range reductions for transcendental functions inside shift-invariant kernels, test replacing exact dual-FMA constant multiplications with single-FMA approximations. Apply mixed-unrolling strategies (e.g., 8x for simple phases, 4x for complex polynomial phases) to balance execution port saturation with register pressure.
